...
### 步骤 3：安装 pytorch
必须从https://pytorch.org/get-started/locally/  安装对应自己设备cuda版本的pytorch，因为不方便将pytorch库写进依赖中。
### 步骤 4：安装ffmpeg
```bash
...
python cli.py --help
```
简单地说，必须使用自动批次（--ab N）或者固定批次（--bs N）参数，其余参数如果不指定将使用默认参数（--fp，--npu，--nts），即默认以fp16精度、不加标点、不返回词级时间戳的方式执行，这是效率最高的模式（显存占用近3G）。
...
下面例子中（包括cli-help.md）出现的所有数字仅仅是占位符，你可以按实际改成任何你想要的整数数字。“audio.mp3”之类的也是占位符，你需要替换成真实的文件地址（-f）或者目录（-d）。
#### 2. 处理单个音频文件（自动批次16）
```bash
...
### Q3: 如何选择批次大小？
A: 首次使用，推荐使用 `--ab 3` 参数启用自动批次管理，系统会根据显存利用率自动调整。然后根据实际结果，使用 `--ab N` 或者固定批次 `--bs N`，N是整数。
